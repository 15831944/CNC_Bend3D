...
//        pBendGL->print();
    ...
    painter.drawPixmap(PAGE_X_MARGIN,PAGE_Y_MARGIN,PAGE_WIDTH,PAGE_HEIGHT/4,pixmap_model);
...
//                             tr("折弯——%1").arg(j+1));
            QImage image1(QString("D:/%1.bmp").arg(j+1+i*4));
            painter.drawImage(QRectF(PAGE_X_MARGIN,PAGE_Y_MARGIN+\
...
{
    point3f temp= pOperate->findBasicData();
    qDebug()<<temp.x;
    qDebug()<<temp.y;
    qDebug()<<temp.z;
